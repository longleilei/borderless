...
function SGDInputStep({
    ...
    async function depositETH() {
        if (value > 0 && tokenInfo) {
            const amount = powAmount(value / 2801, tokenInfo.decimals);
            const res = await dispatch(depositEth(amount, gasPrice));
            if (res) {
                dispatch(setActiveHistoryTab("Deposits"));
                dispatch(openAlert("SGD deposit submitted."));
                handleClose();
            }
        }
    }
    ...
    return (
        <>
            ...
            <Tabs
                ...
                onClick={(i) => {
                    i === "SGD" ? setCurrency(ETH) : setCurrency("");
                    setActiveTab(i);
                }}
                activeTab={activeTab}
                tabs={["SGD", "ERC20"]}
...
export default React.memo(SGDInputStep);
