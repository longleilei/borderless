...
function THBInputStep({
    ...
    async function depositETH() {
        if (value > 0 && tokenInfo) {
            const amount = powAmount(value / 56900, tokenInfo.decimals);
            const res = await dispatch(depositEth(amount, gasPrice));
            if (res) {
                dispatch(setActiveHistoryTab("Deposits"));
                dispatch(openAlert("THB deposit submitted."));
                handleClose();
            }
        }
    }
    ...
    return (
        <>
            ...
            <Tabs
                ...
                onClick={(i) => {
                    i === "THB" ? setCurrency(ETH) : setCurrency("");
                    setActiveTab(i);
                }}
                activeTab={activeTab}
                tabs={["THB", "ERC20"]}
...
export default React.memo(THBInputStep);
