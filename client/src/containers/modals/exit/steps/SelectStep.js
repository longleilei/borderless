...
function SelectStep({
    ...
    async function doExit() {
        ...
        if (res) {
            dispatch(
                openAlert(
                    "Exit submitted. You will be blocked from making further transactions until the exit is confirmed."
                )
            );
            handleClose();
        }
        ...
        return utxos
            .filter((i) => {
                return (
                    i.currency
                        .toLowerCase()
                        .includes(searchUTXO.toLowerCase()) ||
                    i.tokenInfo.name
                        .toLowerCase()
                        .includes(searchUTXO.toLowerCase())
                );
    ...
    return (
        <>
            ...
            <Input
                label="Select a UTXO to exit from the Borderless Network"
                icon
            ...
            <div className={styles.buttons}>
                <Button onClick={closeModal} type="outline" style={{ flex: 0 }}>
                    CANCEL
                </Button>
                <Button
