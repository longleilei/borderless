...
function MergeModal({ open }) {
    ...
    useEffect(() => {
        if (selectedUTXOs.length) {
            ...
            const { typedData } = networkService.getMergeTypedData(
                selectedUTXOs
            );
    ...
    async function submit({ useLedgerSign }) {
        if (selectedUTXOs.length > 1 && selectedUTXOs.length < 5) {
            const res = await dispatch(
                mergeUtxos(useLedgerSign, selectedUTXOs)
            );
            if (res) {
                dispatch(setActiveHistoryTab("Transactions"));
                dispatch(
                    openAlert(
                        "Merge submitted. You will be blocked from making further transactions until the merge is confirmed."
                    )
                );
                handleClose();
            }
        ...
        return (
            <>
                ...
                <div className={styles.disclaimer}>
                    Select the UTXOs you want to merge
                </div>

                <div className={styles.list}>
                    {!utxos.length && (
                        <div className={styles.disclaimer}>
                            You do not have any UTXOs on the Borderless Network.
                        </div>
                    )}
                    {_utxos.map((i, index) => {
                ...
                <div className={styles.disclaimer}>
                    You can select a maximum of 4 UTXOs to merge at once.
                </div>
