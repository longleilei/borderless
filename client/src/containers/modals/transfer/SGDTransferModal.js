...
function SGDTransferModal({ open }) {
        ...
        if (Object.keys(fees).length) {
            const usableFees = balances
                .filter((balance) => {
                    ...
                    if (feeObject) {
                        if (
                            new BN(balance.amount).gte(new BN(feeObject.amount))
                        ) {
                            return true;
                        }
                    }
                ...
                .map((i) => {
                    ...
                    const feeAmount = new BN(feeObject.amount).div(
                        new BN(feeObject.subunit_to_unit)
                    );
                    return {
                        title: i.name,
                        value: i.currency,
    ...
    const selectOptions = balances.map((i) => ({
        title: "SGD",
        value: i.currency,
        subTitle: `Balance: ${logAmount(i.amount * 2801, i.decimals)}`,
    }));
    ...
    async function submit({ useLedgerSign }) {
        if (value > 0 && currency && feeToken && recipient) {
            try {
                ...
                const { txBody, typedData } = await dispatch(
                    getTransferTypedData({
                        utxos: [...selectedUtxos, ...selectedFeeUtxos],
                        recipient,
                        value: powAmount(value / 2801, valueTokenInfo.decimals),
                        currency,
                        feeToken,
                        metadata,
                    })
                ...
                if (res) {
                    ...
                    dispatch(
                        openAlert(
                            "Transfer submitted. You will be blocked from making further transactions until the transfer is confirmed."
                        )
                    );
                    handleClose();
                }
    ...
    const disabledTransfer =
        ...
        !feeToken ||
        !recipient ||
        new BN(value).gt(new BN(getMaxTransferValue() * 2801));

    function getMaxTransferValue() {
        const transferingBalanceObject = balances.find(
            (i) => i.currency.toLowerCase() === currency.toLowerCase()
        );
        ...
        if (currency.toLowerCase() === feeToken.toLowerCase()) {
            ...
            ).minus(new BN(fees[feeToken.toLowerCase()].amount));
            return logAmount(
                availableAmount,
    ...
    function handleUtxoClick(utxo) {
        const isSelected = selectedUtxos.some(
            (i) => i.utxo_pos === utxo.utxo_pos
        );
        if (isSelected) {
            return setSelectedUtxos(
                selectedUtxos.filter((i) => i.utxo_pos !== utxo.utxo_pos)
            );
        }
    ...
    function handleFeeUtxoClick(utxo) {
        const isSelected = selectedFeeUtxos.some(
            (i) => i.utxo_pos === utxo.utxo_pos
        );
        if (isSelected) {
            return setSelectedFeeUtxos(
                selectedFeeUtxos.filter((i) => i.utxo_pos !== utxo.utxo_pos)
            );
        }
        if (selectedUtxos.length + selectedFeeUtxos.length < 4) {
    ...
    function renderUtxoPicker() {
        ...
        const currencyObject = balances.find(
            (i) => i.currency.toLowerCase() === currency.toLowerCase()
        );
        const currencyCoverAmount = new BN(
            powAmount(value.toString(), currencyObject.decimals)
        );
        ...
        const utxoPickerDisabled = sameCurrency
            ? currencyCoverAmount
                  .plus(feeCoverAmount)
                  .gt(selectedCurrencyAmount)
            : currencyCoverAmount.gt(selectedCurrencyAmount) ||
              feeCoverAmount.gt(selectedFeeAmount);

        function renderCurrencyPick() {
            const enough = sameCurrency
                ? currencyCoverAmount
                      .plus(feeCoverAmount)
                      .lte(selectedCurrencyAmount)
                : currencyCoverAmount.lte(selectedCurrencyAmount);

            return (
                <>
                    <div className={styles.description}>
                        Transfer amount to cover:{" "}
                        {sameCurrency
                            ? logAmount(
                                  currencyCoverAmount.plus(feeCoverAmount),
                    ...
                    <div
                        className={[
                            styles.list,
                            !sameCurrency ? styles.doubleList : "",
                        ].join(" ")}
                    >
                        ...
                        {currencyUtxos.map((i, index) => {
                            const selected = selectedUtxos.some(
                                (selected) => selected.utxo_pos === i.utxo_pos
                            );
                            return (
                                <div
                                    key={index}
        ...
        function renderFeePick() {
            const logFeeAmount = new BN(feeObject.amount.toString()).div(
                new BN(feeObject.subunit_to_unit.toString())
            );
            ...
            return (
                <>
                    ...
                    >
                        {!feeUtxos.length && (
                            <div className={styles.disclaimer}>
                                You do not have any fee UTXOs on the OMG
                                Network.
                            </div>
                        )}
                        {feeUtxos.map((i, index) => {
                            const selected = selectedFeeUtxos.some(
                                (selected) => selected.utxo_pos === i.utxo_pos
                            );
                            return (
                                <div
        ...
        return (
            <>
                <h2>Select UTXOs</h2>
                <div className={styles.description}>
                    By default, this wallet will automatically pick UTXOs to
                    cover your transaction amount. However, if you are a more
                    advanced user, you can pick the UTXOs you would like to
                    spend in this transaction manually.
                </div>
                ...
                <div className={styles.disclaimer}>
                    You can select a maximum of 4 UTXOs.
                </div>
        ...
        return (
            <>
                ...
                <InputSelect
                    ...
                    maxValue={getMaxTransferValue() * 2801}
                    flag="sg"
                />
                ...
                {value > 0 && (
                    ...
                    >
                        {selectedUtxos.length
                            ? "Change Selected UTXOs"
                            : "Advanced UTXO Select"}
                    </div>
                )}
...
export default React.memo(SGDTransferModal);
