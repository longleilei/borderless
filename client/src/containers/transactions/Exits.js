...
function Exits({ searchHistory }) {
    ...
    const pendingExits = orderBy(
        useSelector(selectPendingExits, isEqual),
        (i) => i.blockNumber,
        "desc"
    );
    const exitedExits = orderBy(
        useSelector(selectExitedExits, isEqual),
        (i) => i.blockNumber,
        "desc"
    );
    ...
    const renderPending = _pendingExits.map((i, index) => {
        ...
        return (
            <Transaction
                ...
                statusPercentage={
                    i.pendingPercentage <= 100 ? i.pendingPercentage : ""
                }
                title={truncate(i.transactionHash, 6, 4, "...")}
                midTitle={
                    i.exitableAt
                        ? `Exitable ${exitableMoment.format("lll")}`
                        : ""
                }
    ...
    return (
        <>
            ...
            <div className={styles.section}>
                <div className={styles.subTitle}>Withdraws</div>
